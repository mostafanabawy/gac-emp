...
@Component({
  selector: 'app-service-processes',
  standalone: true,
  ...
  styleUrl: './service-processes.component.css'
})
export class ServiceProcessesComponent implements OnInit, AfterViewInit {
  basic = basic;
  hide = true;
  ...
  get ProcessID(): any {
    return this.AddserviceproceduresForm.get('ProcessID');
  }
  ...
  ngOnInit(): void {
    ...
    this.cols = [
      ...
      {
        field: 'ProcessTitleAr',
        title: this.translations?.ProcessTitleAr?.label,
        sort: true,
        visible: true
      },
      {
        field: 'ProcessTitleEn',
        title: this.translations?.ProcessTitleEn?.label,
        sort: true,
        visible: true
      },
      {
    ...
    this.ServiceService.getById(null).subscribe((res) => {
      if (res?.Lookup) {
        this.Processes = res.Lookup.Process;
        this.serachForm.controls['ProcessID'].setValue(this.Processes[0].LookupID);
        if (this.isEditMode) {
          this.AddserviceproceduresForm.controls['ProcessID'].setValue(this.Processes[0].LookupID);
        }
      }
    });
    ...
    this.AddserviceproceduresForm = this.fb.group({
      ProcessID: [[], [Validators.required, nonEmptyArrayValidator]],
      ServiceID: [id],
      ServiceName: [null],
  ...
  fetchData(setting?: any, searchData?: any) {
    ...
    searchData = {
      ServiceID: ServiceID,
      ProcessID: this.serachForm.controls['ProcessID'].value == -1 ? null : this.serachForm.controls['ProcessID'].value,
    };
    ...
    this.ServiceService.getPageWithSearchProcesses(setting, searchData).subscribe({
      next: (res) => {
  ...
  edit(data: any) {
    ...
    this.ServiceService.getByProcessesRowId(Id).subscribe({
      next: (res) => {
        console.log(res)
        if (res && res.items && res.items.length > 0) {
          ...
          this.AddserviceproceduresForm.patchValue({
            ...
            ProcessID: serviceproceduresData.ProcessID,
  ...
  delete(id: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.isLoading = true;
        try {
          await lastValueFrom(this.ServiceService.deleteEServicesServiceProcesses(id));
  ...
  exportTOExcel() {
    ...
    headerMap['ProcessTitleAr'] = this.translations?.ProcessTitleAr?.label || (isArabic ? 'نوع الإجراء (عربي)' : 'Procedure Type (Ar)');
    headerMap['ProcessTitleEn'] = this.translations?.ProcessTitleEn?.label || (isArabic ? 'نوع الإجراء (إنجليزي)' : 'Procedure Type (En)');
    ...
    XLSX.utils.book_append_sheet(wb, ws, isArabic ? 'إجراءات الخدمة' : 'Service Processes');
    ...
    const filename = isArabic ? 'تقرير_إجراءات_الخدمة.xlsx' : 'Service_Processes_Report.xlsx';
  ...
  async onAddAttachmentSubmit() {
    ...
    const ProcessIDs = Array.isArray(formData.ProcessID) ? formData.ProcessID : [formData.ProcessID];
    ...
    ProcessIDs.forEach((ProcessID: any) => {
      ...
      requests.push(
        this.isEditMode
          ? this.ServiceService.updateEServicesServiceProcesses(payload)
          : this.ServiceService.addEServicesServiceProcesses(payload)
      );
    });
  ...
  handleActionClick(event: any) {
    ...
    if (event && event.row && event.row.RowId) {
  ...
  toggleRowExpand(serviceId: any) {
    ...
    if (this.expandedRow?.RowId === serviceId) {
      ...
      if (this.expandedRow) {
        this.expandedRowDetails = [
          {
            label: this.translations?.ProcessTitleAr?.label,
            value: this.expandedRow.ProcessTitleAr,
            fullWidth: false
          },
          {
            label: this.translations?.ProcessTitleEn?.label,
            value: this.expandedRow.ProcessTitleEn,
            fullWidth: false
          }
        ];
      }
