...
@Component({
  ...
  styleUrl: './service-statuses.component.css'
})
export class ServiceStatusesComponent implements OnInit, AfterViewInit {
  basic = basic;
  hide = true;
  ...
  get StatusID(): any {
    return this.AddserviceproceduresForm.get('StatusID');
  }
  ...
  ngOnInit(): void {
    ...
    this.cols = [
      ...
      {
        field: 'StatusTitleAr',
        title: this.translations?.StatusTitleAr?.label,
        sort: true,
        visible: true
      },
      {
        field: 'StatusTitleEn',
        title: this.translations?.StatusTitleEn?.label,
        sort: true,
        visible: true
      },
      {
    ...
    this.ServiceService.getById(null).subscribe((res) => {
      if (res?.Lookup) {
        this.Status = res.Lookup.Statuses;
        this.serachForm.controls['StatusID'].setValue(this.Status[0].LookupID);
        if (this.isEditMode) {
          this.AddserviceproceduresForm.controls['StatusID'].setValue(this.Status[0].LookupID);
        }
      }
    });
    ...
    this.AddserviceproceduresForm = this.fb.group({
      StatusID: [[], [Validators.required, nonEmptyArrayValidator]],
      ServiceID: [id, Validators.required],
      ServiceName: [null],
  ...
  fetchData(setting?: any, searchData?: any) {
    ...
    searchData = {
      ServiceID: ServiceID,
      StatusID: this.serachForm.controls['StatusID'].value == -1 ? null : this.serachForm.controls['StatusID'].value,
    };
    ...
    this.ServiceService.getPageWithSearchStatuses(setting, searchData).subscribe({
      next: (res) => {
  ...
  edit(data: any) {
    ...
    this.ServiceService.getByRowId(Id).subscribe({
      next: (res) => {
        if (res && res.items && res.items.length > 0) {
          ...
          this.AddserviceproceduresForm.patchValue({
            ...
            StatusID: serviceproceduresData.StatusID,
  ...
  exportTOExcel() {
    ...
    headerMap['StatusTitleAr'] = this.translations?.StatusTitleAr?.label || (isArabic ? 'الحالة (عربي)' : 'Status (Ar)');
    headerMap['StatusTitleEn'] = this.translations?.StatusTitleEn?.label || (isArabic ? 'الحالة (إنجليزي)' : 'Status (En)');
    ...
    XLSX.utils.book_append_sheet(wb, ws, isArabic ? 'حالات الخدمة' : 'Service Statuses');
    ...
    const filename = isArabic ? 'تقرير_حالات_الخدمة.xlsx' : 'Service_Statuses_Report.xlsx';
  ...
  toggleRowExpand(serviceId: any) {
    ...
    if (this.expandedRow?.RowId === serviceId) {
      ...
      if (this.expandedRow) {
        this.expandedRowDetails = [
          {
            label: this.translations?.StatusTitleAr?.label,
            value: this.expandedRow.StatusTitleAr,
            fullWidth: false
          },
          {
            label: this.translations?.StatusTitleEn?.label,
            value: this.expandedRow.StatusTitleEn,
            fullWidth: false
          },
          {
            label: this.translations?.ServiceTitleAr?.label,
            value: this.expandedRow.ServiceTitleAr,
            fullWidth: false
          },
          {
            label: this.translations?.ServiceTitleEn?.label,
            value: this.expandedRow.ServiceTitleEn,
            fullWidth: false
          }
        ];
      }
  ...
  async onAddAttachmentSubmit() {
    ...
    const StatusIDs = Array.isArray(formData.StatusID) ? formData.StatusID : [formData.StatusID];
    ...
    StatusIDs.forEach((StatusId: any) => {
      const payload: any = {
        ServiceID: serviceID,
        StatusID: StatusId,
      ...
      requests.push(
        this.isEditMode
          ? this.ServiceService.updateEServicesServiceStatuses(payload)
          : this.ServiceService.addEServicesServiceStatuses(payload)
      );
    });
