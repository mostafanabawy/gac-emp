...
export class MessagesComponent implements OnInit, AfterViewInit {
  ...
  get MessageTemplateAR(): any {
    return this.AddserviceproceduresForm.get('MessageTemplateAR');
  }

  get MessageTemplateEn(): any {
    return this.AddserviceproceduresForm.get('MessageTemplateEn');
  }
  ...
  ngOnInit(): void {
    this.translations =
      this.localizationService.getTranslationsByScreen(
        'global',
        'messagesettings'
      );
    ...
    this.cols = [
      ...
      {
        field: 'MessageCode',
        title: this.translations?.MessageCode?.label || 'كود الرسالة',
        sort: true,
        visible: true
      },
      {
      ...
      {
        field: 'MessageTemplateAR',
        title: this.translations?.MessageTemplateAR?.label,
        sort: true,
        visible: false,
        html: true,
        cellRenderer: (row: any) => {
          const text = row.MessageTemplateAR || '';
          const maxLength = 100;
          if (text.length > maxLength) {
            return `<div class="whitespace-normal break-words max-w-md" title="${text}">${text.substring(0, maxLength)}...</div>`;
          }
          return `<div class="whitespace-normal break-words max-w-md">${text}</div>`;
        }
      },
      {
        field: 'MessageTemplateEn',
        title: this.translations?.MessageTemplateEn?.label,
        sort: true,
        visible: false,
        html: true,
        cellRenderer: (row: any) => {
          const text = row.MessageTemplateEn || '';
          const maxLength = 100;
          if (text.length > maxLength) {
            return `<div class="whitespace-normal break-words max-w-md" title="${text}">${text.substring(0, maxLength)}...</div>`;
          }
    ...
    this.MessagesService.getById(null).subscribe((res) => {
    ...
    this.AddserviceproceduresForm = this.fb.group({
      ...
      MessageTemplateAR: [null, [Validators.required]],
      MessageTemplateEn: [null, [Validators.required]],
  ...
  fetchData(setting?: any, searchData?: any) {
    ...
    searchData = {
      MessageTemplateAR: formValues.MessageTemplateAR || null,
      MessageTemplateEn: formValues.MessageTemplateEn || null,
    ...
    this.MessagesService.getPageWithSearch(setting, searchData).subscribe({
      next: (res) => {
  ...
  customSearchFn(term: string, item: any) {
    ...
    return item.MessageTemplateAR.toLowerCase().includes(term) ||
      item.MessageTemplateEn.toLowerCase().includes(term);
  }
  ...
  edit(data: any) {
    ...
    this.MessagesService.getByMessageID(Id).subscribe({
      next: (res) => {
        console.log(res)
        if (res && res.items && res.items.length > 0) {
          ...
          this.AddserviceproceduresForm.patchValue({
            ...
            MessageTemplateAR: serviceproceduresData.MessageTemplateAR || '',
            MessageTemplateEn: serviceproceduresData.MessageTemplateEn || '',
  ...
  exportTOExcel() {
    ...
    headerMap['MessageTemplateAR'] = this.translations?.MessageTemplateAR?.label || (isArabic ? 'نص الرسالة (عربي)' : 'Message Template (Ar)');
    headerMap['MessageTemplateEn'] = this.translations?.MessageTemplateEn?.label || (isArabic ? 'نص الرسالة (إنجليزي)' : 'Message Template (En)');
    ...
    XLSX.utils.book_append_sheet(wb, ws, isArabic ? 'إعدادات الرسائل' : 'Message Settings');
    ...
    const filename = isArabic ? 'تقرير_إعدادات_الرسائل.xlsx' : 'Message_Settings_Report.xlsx';
  ...
  onAddAttachmentSubmit() {
    ...
    Swal.fire({
      ...
      const requests = serviceIDs.map((serviceID: number) => {
        const payload: any = {
          MessageTemplateAR: formData.MessageTemplateAR || null,
          MessageTemplateEn: formData.MessageTemplateEn || null,
        ...
        return this.isEditMode
          ...
          : this.MessagesService.add(payload);
      });
  ...
  toggleRowExpand(MessageId: any) {
    ...
    if (this.expandedRow?.MessageId === MessageId) {
      ...
      if (this.expandedRow) {
        this.expandedRowDetails = [
          {
            label: this.translations?.MessageTemplateAR?.label,
            value: this.expandedRow.MessageTemplateAR,
            fullWidth: true // لجعلها تأخذ العرض الكامل
          },
          {
            label: this.translations?.MessageTemplateEn?.label,
            value: this.expandedRow.MessageTemplateEn,
            fullWidth: true // لجعلها تأخذ العرض الكامل
          }
        ];
      }
  ...
  toggleColumn(field: string) {
    ...
    if (colInCols && !colInCols.fixed) {
      colInCols.visible = !colInCols.visible;
    }
