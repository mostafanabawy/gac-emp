...
@Component({
  ...
  templateUrl: './duty-free-rpt.component.html',
  styleUrl: './duty-free-rpt.component.css',
  changeDetection: ChangeDetectionStrategy.OnPush
})

export class DutyFreeRPTComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.cols = [
      ...
      {
        field: 'FeesValue',
        title: this.isArabic ? 'قيمة الرسوم' : 'Fees Value',
        sort: true,
        visible: true,
        width: '150px'
      },
      {
        field: this.isArabic ? 'DutyFreeAr' : 'DutyFreeEn',
        title: this.isArabic ? 'نوع الإعفاء' : 'Exemption Type',
        sort: true,
        visible: true,
        width: '200px'
      },
      {
        field: 'ExemptionResone',
        title: this.isArabic ? 'سبب الإعفاء' : 'Exemption Reason',
        sort: true,
        visible: true,
        width: '250px'
      },
      ...
      // حالة الطلب
      {
        ...
        title: this.isArabic ? 'الحالة' : 'Status',
        sort: true,
        visible: false,
        width: '180px'
      },
      ...
      {
        field: 'ExemptionDateFrom',
        title: this.isArabic ? 'بداية الإعفاء' : 'Exemption From',
        sort: true,
        visible: false,
        width: '160px'
      },
      {
        field: 'ExemptionDateTo',
        title: this.isArabic ? 'نهاية الإعفاء' : 'Exemption To',
        sort: true,
        visible: false,
        width: '160px'
      }
  ...
  search(collapseSearch: boolean = false) {
    ...
    const searchPayload = {
      ...
      ExemptionResone: formValue.ExemptionResone,
      DutyFree: formValue.DutyFree,
      ExemptionDateFrom: this.formatDate(formValue.ExemptionDateFrom),
      ExemptionDateTo: this.formatDate(formValue.ExemptionDateTo)
    };
    ...
    this.LicensesService.searchGetDutyFreeRPT(searchPayload).subscribe(
      data => {
  ...
  hasAdvancedSearchValues(formValue: any): boolean {
    // الحقول المتقدمة للتقرير الإعفاء الجمركي
    const advancedFields = [
      ...
      'ExemptionResone', 'DutyFree', 'FeesValue',
  ...
  buildSearchCriteria(formValue: any) {
    ...
    const textFields = [
      ...
      { key: 'ExemptionResone', label: this.isArabic ? 'سبب الإعفاء' : 'Exemption Reason' }
    ...
    if (formValue.FkStatusID) {
      const selectedStatus = this.requestStatuses.find((s: any) => s.LookupID === formValue.FkStatusID);
      if (selectedStatus) {
        this.searchCriteria.push({
          label: t?.Status?.label || (this.isArabic ? 'الحالة' : 'Status'),
          value: this.isArabic ? selectedStatus.TitleAr : selectedStatus.TitleEn,
          key: 'FkStatusID'
        });
      }
    }
    ...
    // نوع الإعفاء
    if (formValue.DutyFree && formValue.DutyFree !== -1) {
      const selectedDutyFree = this.dutyFreeTypes.find((d: any) => d.LookupID === formValue.DutyFree);
      if (selectedDutyFree) {
        this.searchCriteria.push({
          label: this.isArabic ? 'نوع الإعفاء' : 'Exemption Type',
          value: this.isArabic ? selectedDutyFree.TitleAr : selectedDutyFree.TitleEn,
          key: 'DutyFree'
        });
      }
    }
    ...
    // تواريخ الإعفاء
    if (formValue.ExemptionDateFrom || formValue.ExemptionDateTo) {
      const fromDate = formValue.ExemptionDateFrom || '';
      const toDate = formValue.ExemptionDateTo || '';
      this.searchCriteria.push({
        label: this.isArabic ? 'تاريخ الإعفاء' : 'Exemption Date',
        value: `${fromDate} ${fromDate && toDate ? '-' : ''} ${toDate}`.trim(),
        key: 'ExemptionDate'
      });
    }
  ...
  exportToExcel() {
    ...
    // جلب كل البيانات
    this.LicensesService.searchGetDutyFreeRPT(exportPayload).subscribe(
      data => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, this.isArabic ? 'تقرير الإعفاء الجمركي' : 'Duty Free Report');
        ...
        const filename = this.isArabic ? 'تقرير_الإعفاء_الجمركي.xlsx' : 'Duty_Free_Report.xlsx';
