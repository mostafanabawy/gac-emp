...
export class RequestDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(params => {
      console.log('Received URL params in RequestDetails:', params);

      // Check if we have any filter parameters
      const filterParams = [
        'CreationDateFrom', 'CreationDateTo', 'ServiceID', 'SubmissionWay', 'FkClubID',
        'DepartmentID', 'SectionID', 'RequestTypeID', 'FkStatusID', 'FkProcessID',
        'ApplicationNumber', 'ApprovedLicense', 'LicenseStatus',
        'RequestDateFrom', 'RequestDateTo',
        'RequestCategory', 'ApprovalDateFrom', 'ApprovalDateTo', 'RejectDateFrom', 'RejectDateTo'
      ];
      ...
      this.LicensesService.getById(null).subscribe(data => {
        ...
        this.requestCategories = data.Lookup?.RequestCategories || [];
        ...
        if (hasFilters) {
          ...
          filterParams.forEach(param => {
            if (params[param]) {
              ...
              if (['DepartmentID', 'SectionID', 'ServiceID', 'RequestTypeID', 'SubmissionWay', 'FkClubID', 'FkStatusID', 'FkProcessID', 'RequestCategory'].includes(param)) {
          ...
          console.log('Applied filters from URL in RequestDetails:', patchData);
    ...
    this.cols = [
      ...
      { field: this.isArabic ? 'SubmissionWayAr' : 'SubmissionWayEn', title: t?.submissionWay?.label || (this.isArabic ? 'طريقة التقديم' : 'Submission Method'), sort: true, visible: true, width: '150px' },
      ...
      { field: 'ApprovedLicense', title: t?.licenseNumber?.label, sort: true, visible: false, width: '180px', isNumber: true },
    ];
  ...
  setupFormValueCleaning() {
    const multiSelectFields = ['DepartmentID', 'SectionID', 'ServiceID', 'RequestTypeID', 'FkStatusID', 'FkProcessID', 'SubmissionWay', 'FkClubID', 'RequestCategory'];
  ...
  hasAdvancedSearchValues(formValue: any): boolean {
    ...
    const advancedFields = [
      ...
      'SubmissionWay', 'ApplicationNumber', 'ApprovedLicense', 'CreationDate',
      'LicenseFirstIssueDateFrom', 'LicenseFirstIssueDateTo',
      'LicenseStartDateFrom', 'LicenseStartDateTo',
      'LicenseEndDateFrom', 'LicenseEndDateTo',
      'RequestDateFrom', 'RequestDateTo'
    ];
  ...
  buildSearchCriteria(formValue: any) {
    ...
    if (Array.isArray(formValue.SubmissionWay) && formValue.SubmissionWay.length > 0) {
      const selectedSubmissions = this.submissionMethods.filter((s: any) => formValue.SubmissionWay.includes(s.LookupID));
      if (selectedSubmissions.length > 0) {
        const values = selectedSubmissions.map((s: any) => this.isArabic ? s.TitleAr : s.TitleEn).join(', ');
        this.searchCriteria.push({
    ...
    // Request Category
    if (Array.isArray(formValue.RequestCategory) && formValue.RequestCategory.length > 0) {
      const selectedCategories = this.requestCategories.filter((c: any) => formValue.RequestCategory.includes(c.LookupID));
      if (selectedCategories.length > 0) {
        const values = selectedCategories.map((c: any) => this.isArabic ? c.TitleAr : c.TitleEn).join(', ');
        this.searchCriteria.push({
          label: this.isArabic ? 'تصنيف الطلبات' : 'Request Category',
          value: values,
          key: 'RequestCategory'
        });
      }
    ...
    // Approval Dates
    if (formValue.ApprovalDateFrom || formValue.ApprovalDateTo) {
      const fromDate = formValue.ApprovalDateFrom || '';
      const toDate = formValue.ApprovalDateTo || '';
      this.searchCriteria.push({
        label: this.isArabic ? 'تاريخ الموافقة' : 'Approval Date',
        value: `${fromDate} ${this.isArabic ? 'إلى' : 'to'} ${toDate}`,
        key: 'ApprovalDate'
      });
    }

    // Rejection Dates
    if (formValue.RejectDateFrom || formValue.RejectDateTo) {
      const fromDate = formValue.RejectDateFrom || '';
      const toDate = formValue.RejectDateTo || '';
      this.searchCriteria.push({
        label: this.isArabic ? 'تاريخ الرفض' : 'Rejection Date',
        value: `${fromDate} ${this.isArabic ? 'إلى' : 'to'} ${toDate}`,
        key: 'RejectDate'
      });
    }
    ...
    if (formValue.LicenseFirstIssueDateFrom || formValue.LicenseFirstIssueDateTo) {
      const fromDate = formValue.LicenseFirstIssueDateFrom || '';
      const toDate = formValue.LicenseFirstIssueDateTo || '';
      this.searchCriteria.push({
        label: t?.licenseFirstIssueDate?.label || (this.isArabic ? 'تاريخ أول إصدار للرخصة' : 'License First Issue Date'),
        value: `${fromDate} ${fromDate && toDate ? '-' : ''} ${toDate}`.trim(),
        key: 'LicenseFirstIssueDate'
      });
    }

    if (formValue.LicenseStartDateFrom || formValue.LicenseStartDateTo) {
      const fromDate = formValue.LicenseStartDateFrom || '';
      const toDate = formValue.LicenseStartDateTo || '';
      this.searchCriteria.push({
        label: t?.licenseStartDate?.label || (this.isArabic ? 'تاريخ بداية الرخصة' : 'License Start Date'),
        value: `${fromDate} ${fromDate && toDate ? '-' : ''} ${toDate}`.trim(),
        key: 'LicenseStartDate'
      });
    }

    if (formValue.LicenseEndDateFrom || formValue.LicenseEndDateTo) {
      const fromDate = formValue.LicenseEndDateFrom || '';
      const toDate = formValue.LicenseEndDateTo || '';
      this.searchCriteria.push({
        label: t?.licenseEndDate?.label || (this.isArabic ? 'تاريخ نهاية الرخصة' : 'License End Date'),
        value: `${fromDate} ${fromDate && toDate ? '-' : ''} ${toDate}`.trim(),
        key: 'LicenseEndDate'
      });
    }

    if (formValue.RequestDateFrom || formValue.RequestDateTo) {
      const fromDate = formValue.RequestDateFrom || '';
      const toDate = formValue.RequestDateTo || '';
      this.searchCriteria.push({
        label: t?.requestDate?.label || (this.isArabic ? 'تاريخ التقديم' : 'Request Date'),
        value: `${fromDate} ${fromDate && toDate ? '-' : ''} ${toDate}`.trim(),
        key: 'RequestDate'
      });
    }
  ...
  removeCriteria(key: string) {
    ...
    if (key === 'CreationDate') {
    ...
    } else if (key === 'LicenseFirstIssueDate') {
      this.searchForm.patchValue({
        LicenseFirstIssueDateFrom: null,
        LicenseFirstIssueDateTo: null
      });
    } else if (key === 'LicenseStartDate') {
      this.searchForm.patchValue({
        LicenseStartDateFrom: null,
        LicenseStartDateTo: null
      });
    } else if (key === 'LicenseEndDate') {
      this.searchForm.patchValue({
        LicenseEndDateFrom: null,
        LicenseEndDateTo: null
      });
      ...
      if (control) {
        ...
        if (['DepartmentID', 'SectionID', 'ServiceID', 'RequestTypeID', 'FkStatusID', 'FkProcessID', 'SubmissionWay'].includes(key)) {
          control.setValue([]);
  ...
  clearDropdown(fieldName: string) {
    ...
    if (control) {
      // For multi-select fields, set to empty array
      if (['DepartmentID', 'SectionID', 'ServiceID', 'RequestTypeID', 'FkStatusID', 'FkProcessID', 'SubmissionWay'].includes(fieldName)) {
        control.setValue([]);
      } else {
        control.setValue(-1);
      }
  ...
  exportToExcel() {
    ...
    this.LicensesService.search(exportPayload).subscribe(
      data => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, this.isArabic ? 'تفاصيل الطلبات' : 'Request Details');
        ...
        const filename = this.isArabic ? 'تقرير_تفاصيل_الطلبات.xlsx' : 'Request_Details_Report.xlsx';
